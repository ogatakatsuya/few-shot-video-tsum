"""Generate few-shot summary using most dissimilar examples."""
...
from src.retrieval.operations import search_dissimilar_videos
...
def retrieve_dissimilar_videos(
    conn: connection,
    ...
    """
    Retrieve most dissimilar videos using RAG.

    Args:
        conn: Database connection
        encoder: Video encoder
        video_path: Path to query video
        top_k: Number of dissimilar videos to retrieve
    ...
    print(f"  Retrieving top-{top_k} dissimilar videos...")
    retrieved = search_dissimilar_videos(conn, query_embedding, limit=top_k)
...
def main():
    ...
    output_csv = Path(f"results/distant-few-shot/summaries_output_{now}.csv")
    ...
    for i, video_data in enumerate(selected_videos, 1):
        ...
        try:
            # Retrieve dissimilar videos
            retrieved = retrieve_dissimilar_videos(
